import { Container } from "@/components/container";
import { Hero } from "@/components/hero";
import { SiteHeader } from "@/components/site-header";

/*
 * Home page. A single screen holding nothing but the portrait, the name, the
 * role and the introduction. There is no content below the fold, so the page
 * does not scroll: the header navigation is the only way onward, and it
 * always lists every CV section.
 *
 * The root layout makes body a full height flex column, so flex-1 here takes
 * the space left by the header and centres the block inside it.
 */
export default function Home() {
  return (
    <>
      <SiteHeader />

      <Container className="flex flex-1 items-center">
        <main className="w-full py-12">
          <Hero />
        </main>
      </Container>
    </>
  );
}
